...
export default {
	...
	decode: (payload) => {
		...
		for (let i = 0; i < target_bssid_list_count; i++) {
			...
			const target_bss_channel_number = ord(payload, offset++);
	...
	encode: (tlv) => {
		...
		for (let item in tlv.target_bssid_list) {
			...
			if (type(item.target_bss_channel_number) != "int" || item.target_bss_channel_number < 0 || item.target_bss_channel_number > 0xff)
				return null;
			...
			push(fmt, "B");
			push(val, item.target_bss_channel_number);
