...
export default {
	...
	schema: {
		...
		properties: {
			...
			neighbor_bsscolor_in_use_bitmap: {
				type: "integer",
				required: true,
				minimum: 0
			}
	...
	decode: (payload) => {
		...
		const neighbor_bsscolor_in_use_bitmap = unpack('!Q', payload, offset);
		offset += 8;
	...
	encode: (tlv) => {
		...
		if (type(tlv.partial_bss_color) != "bool")
			return null;
		...
		if (type(tlv.neighbor_bsscolor_in_use_bitmap) != "int" || tlv.neighbor_bsscolor_in_use_bitmap < 0 || tlv.neighbor_bsscolor_in_use_bitmap > 0xffffffffffffffff)
			return null;
		...
		push(fmt, "!Q");
		push(val, tlv.neighbor_bsscolor_in_use_bitmap);
